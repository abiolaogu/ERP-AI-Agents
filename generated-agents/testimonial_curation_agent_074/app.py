...
Auto-generated from catalog definition: testimonial_curation_agent_074
"""
...
# Configuration
class Config:
    APP_NAME = "testimonial-curation-agent-074"
    VERSION = "1.0.0"
    PORT = 8274
...
# Service
class TestimonialCurationAgentService:
    """Main agent service"""
...
# Application
app = FastAPI(
    title="Testimonial Curation Agent",
    description="Selects and formats customer quotes",
    version=config.VERSION
)

service = TestimonialCurationAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Testimonial Curation Agent",
        "version": config.VERSION,
        "status": "operational",
