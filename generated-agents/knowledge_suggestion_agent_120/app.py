...
Auto-generated from catalog definition: knowledge_suggestion_agent_120
"""
...
# Configuration
class Config:
    APP_NAME = "knowledge-suggestion-agent-120"
    VERSION = "1.0.0"
    PORT = 8320
...
# Service
class KnowledgeSuggestionAgentService:
    """Main agent service"""
...
# Application
app = FastAPI(
    title="Knowledge Suggestion Agent",
    description="Suggests relevant help articles in chat",
    version=config.VERSION
)

service = KnowledgeSuggestionAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Knowledge Suggestion Agent",
        "version": config.VERSION,
        "status": "operational",
