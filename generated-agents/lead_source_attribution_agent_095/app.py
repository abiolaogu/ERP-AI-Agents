"""
Lead Source Attribution Agent
Explains where leads really came from

Auto-generated from catalog definition: lead_source_attribution_agent_095
"""
...
# Configuration
class Config:
    APP_NAME = "lead-source-attribution-agent-095"
    VERSION = "1.0.0"
    PORT = 8295
...
# Service
class LeadSourceAttributionAgentService:
    """Main agent service"""
...
# Application
app = FastAPI(
    title="Lead Source Attribution Agent",
    description="Explains where leads really came from",
    version=config.VERSION
)

service = LeadSourceAttributionAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Lead Source Attribution Agent",
        "version": config.VERSION,
        "status": "operational",
