...
Searches internal docs for support staff

Auto-generated from catalog definition: documentation_navigator_agent_135
"""
...
# Configuration
class Config:
    APP_NAME = "documentation-navigator-agent-135"
    VERSION = "1.0.0"
    PORT = 8335
...
# Service
class DocumentationNavigatorAgentService:
    """Main agent service"""
...
# Application
app = FastAPI(
    title="Documentation Navigator Agent",
    description="Searches internal docs for support staff",
    version=config.VERSION
)

service = DocumentationNavigatorAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Documentation Navigator Agent",
        "version": config.VERSION,
        "status": "operational",
