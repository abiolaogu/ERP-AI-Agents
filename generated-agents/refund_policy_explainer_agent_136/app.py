...
Auto-generated from catalog definition: refund_policy_explainer_agent_136
"""
...
# Configuration
class Config:
    APP_NAME = "refund-policy-explainer-agent-136"
    VERSION = "1.0.0"
    PORT = 8336
...
# Service
class RefundPolicyExplainerAgentService:
    """Main agent service"""
...
# Application
app = FastAPI(
    title="Refund Policy Explainer Agent",
    description="Clarifies terms to customers",
    version=config.VERSION
)

service = RefundPolicyExplainerAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Refund Policy Explainer Agent",
        "version": config.VERSION,
        "status": "operational",
