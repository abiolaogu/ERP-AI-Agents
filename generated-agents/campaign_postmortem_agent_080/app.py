...
Auto-generated from catalog definition: campaign_postmortem_agent_080
"""
...
# Configuration
class Config:
    APP_NAME = "campaign-postmortem-agent-080"
    VERSION = "1.0.0"
    PORT = 8280
...
# Application
app = FastAPI(
    title="Campaign Post-mortem Agent",
    description="Summarizes what worked/failed in campaigns",
    version=config.VERSION
)

service = CampaignAgentService(config.CLAUDE_API_KEY, config.CLAUDE_MODEL)
...
async def root():
    return {
        "agent_id": config.APP_NAME,
        "name": "Campaign Post-mortem Agent",
        "version": config.VERSION,
        "status": "operational",
